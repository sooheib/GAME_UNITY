...
public class GameController : MonoBehaviour {
  ...
  public GUIText scoreText;
	...
	public string Picture = "https://spartaapp.azurewebsites.net/icon.png";
  ...
  public Texture twitterImg, fbImage;
  ...
  void OnGUI () {
    ...
    if (!paused) {
      string shareTxt = "I%20ran%20" + score + "%20clicks%20through%20RED!%20%23dodgered%0ESPRITMOBILE%0A";
...
//      GUILayout.Space(40);
  ...
  void OnApplicationPause(bool inIsPause) {
	...
	IEnumerator Facebook(int score) {
...
//                          "&redirect_uri=" + WWW.EscapeURL("http://www.facebook.com/"));
			...
			Caption = "I just got "+ score +"score friends! Can you beat it?";
			Application.OpenURL("https://www.facebook.com/dialog/feed?"+ "app_id="+AppID+"&link="+
				...
				ReplaceSpace(Caption)+ "&description="+ReplaceSpace(Description)+"&redirect_uri=https://facebook.com/");
		}
 }
  ...
  void Resume () {
    ...
    nobodyEscapesText.text = Localise.translate("\nGAME\nOVER");
		gameOver = true;
    ...
    Vector3 spawnPosition;
      ...
      for (int j = 0; player != null && j < maxHazardCount; j++) {
        if (!hazards[j].activeInHierarchy) {
          ...
          float rand = Random.Range(0.1f, 2.5f);
          if (rand < 1) {
						//z ViewportToWorldPoint transforms an x-y screen position into a x-y-z position in 3D space.
            spawnPosition = Camera.main.ViewportToWorldPoint(new Vector3(-0.1f, rand, 0));
          } else if (rand <2) {
            spawnPosition = Camera.main.ViewportToWorldPoint(new Vector3(rand - 1, 2.1f, 0));
          } else {
            spawnPosition = Camera.main.ViewportToWorldPoint(new Vector3(1.1f, rand - 2 + 1f, 0));
          }
